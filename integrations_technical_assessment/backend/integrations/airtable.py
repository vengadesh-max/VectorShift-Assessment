...
async def get_items_hubspot(credentials, object_type="contacts"):
    credentials = json.loads(credentials)
    ...
    if response.status_code != 200:
        raise HTTPException(
            ...
            detail=f"Failed to fetch HubSpot {object_type}.",
        )
        ...
        integration_items.append(
            IntegrationItem(
                ...
                name=(
                    ...
                    else item.get("properties", {}).get("firstname", "")
                    + " "
...
async def get_hubspot_items_endpoint(
    credentials: str = Form(...), object_type: str = Form("contacts")
):
    return await get_items_hubspot(credentials, object_type)
